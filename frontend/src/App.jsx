...
export default function App() {
  ...
  const renderView = () => {
    ...
    // 2. User & Attendee: Events Gallery (for users - clean layout without admin dropdown)
    if (activeTab === 'events') {
      return (
        <PageTransition key="user-events" className="min-h-screen bg-transparent font-sans text-slate-900 dark:text-zinc-50 flex flex-col selection:bg-purple-100">
          {/* User Top Header */}
          <header className="bg-white/80 dark:bg-zinc-900/60 backdrop-blur-xl border-b border-slate-100 dark:border-zinc-800/40 sticky top-0 z-40">
            <div className="max-w-[1400px] mx-auto px-6 h-16 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Logo onClick={() => setActiveTab(user ? 'organizer' : 'events')} />
              ...
              <div className="flex items-center gap-3">
                ...
                {user ? (
                  ...
                  >
                    <Shield className="w-3.5 h-3.5 text-[#6e2b8b] dark:text-[#da7756]" />
